...
describe("record-store", () => {
  ...
  describe("calcMisrate", () => {
    it("returns zero when no records", () => {
      fsState.exists = true;
      fsState.content = "[]";
      expect(calcMisrate()).toEqual({ rate: 0, totalDecided: 0, overridden: 0 });
    });

    it("calculates rate as overridden / (passed + rejected)", () => {
      fsState.exists = true;
  ...
  describe("checkMisrateAlert", () => {
    it("returns null when few decisions", () => {
      fsState.exists = true;
      fsState.content = JSON.stringify([
        mkRecord({ prId: "PR-1", decision: "reject", humanOverridden: true }),
        mkRecord({ prId: "PR-2", decision: "pass" }),
        mkRecord({ prId: "PR-3", decision: "pass" }),
      ]);
      // totalDecided=3 < 5, 不告警
    ...
    it("returns alert when misrate above threshold", () => {
      ...
      records.push(mkRecord({ prId: "PR-5", decision: "pass" }));
      fsState.exists = true;
    ...
    it("returns null when misrate below threshold", () => {
      const records: ReviewRecord[] = [];
      for (let i = 0; i < 10; i++) {
        records.push(mkRecord({ prId: `PR-${i}`, decision: "pass" }));
      }
      records.push(mkRecord({ prId: "PR-10", decision: "reject", humanOverridden: false }));
      records.push(mkRecord({ prId: "PR-11", decision: "reject", humanOverridden: true }));
      fsState.exists = true;
      fsState.content = JSON.stringify(records);
      // 1/12 = 8.3% < 15%
      expect(checkMisrateAlert()).toBeNull();
